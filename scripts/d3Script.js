...
const sortByAttribute = (array, attribute) =>
  array.slice().sort((a, b) => b[attribute] - a[attribute]);

// Function to fetch the data from the JSON file
async function fetchData() {
  const response = await (await fetch("data/data.json")).json();
  const converted = response.map((e) =>
    Object.assign(
      {},
      {
        ...e,
...
async function createTop3CryptoByVolumePieChart() {
  ...
  const other_volume = data
    .slice(3)
    .reduce((sum, currency) => sum + currency.volume, 0);

  // Create a new array for the pie chart data, including the top 3 and "Others"
  const pie_data = [...top_3data, { name: "Others", volume: other_volume }];
  ...
  slices
    ...
    .on("mouseover", function (d) {
      ...
      d3.select("#tooltip")
        .style("visibility", "visible")
        .html(
          `<strong>${d.data.name}</strong><br>Percentage: ${percentage.toFixed(
            1
          )}%`
        );
    })
